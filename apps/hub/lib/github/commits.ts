...
export async function syncCommits(owner: string, repo: string, repositoryId: string, limit: number = 100): Promise<number> {
      ...
      try {
        await prisma.commit.upsert({
          ...
          update: {
            ...
            committerDate: commit.commit.committer?.date ? new Date(commit.commit.committer.date) : new Date(),
...
export async function syncAllCommits(limit: number = 100, repositoryIds?: string[]): Promise<number> {
  let totalSynced = 0;
