...
export const ListboxButton = forwardRef<HTMLButtonElement, ListboxButtonProps>(
    ({ className = "", ...props }, ref) => {
        const baseStyles = "w-full flex items-center justify-between gap-2 px-3 py-2 text-sm font-medium text-neutral-900 bg-neutral-50 hover:bg-neutral-100 border border-neutral-200 rounded-lg transition-colors outline-hidden data-focus:ring-2 data-focus:ring-primary-500 data-focus:border-primary-500";
        return <HeadlessListboxButton ref={ref} className={cn(baseStyles, className)} {...props} />;
    });
...
export const ListboxOptions = forwardRef<HTMLDivElement, ListboxOptionsProps>(
    ({ className = "", ...props }, ref) => {
        const baseStyles = "absolute z-120 mt-1 w-full max-h-60 overflow-auto rounded-lg bg-panel py-1 shadow-sm-lg ring-1 ring-black ring-opacity-5 focus:outline-hidden";
        return <HeadlessListboxOptions ref={ref} className={cn(baseStyles, className)} {...props} />;
    });
...
export const ListboxOption = forwardRef(<TValue,>(
    ...
    const baseStyles = "relative cursor-pointer select-none py-2 px-4 text-sm text-neutral-700 data-focus:bg-neutral-100 data-focus:text-neutral-900 data-selected:bg-primary-50 data-selected:text-primary-700";
    return <HeadlessListboxOption ref={ref} className={cn(baseStyles, className)} {...props} />;
}) as <TValue>(
